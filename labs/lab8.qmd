---
title: "Lab 8: Dealing with Missing Data"
author: 
  - FirstName LastName
...
Another option under the MAR assumption is to estimate a propensity score, and then use inverse probability weighting (IPW) to adjust results. The original model already includes a regression specification with covariates, so a "pure" IPW estimation doesn't make much sense. Instead, the code below combines regression with IPW, which is equivalent to implementing *doubly robust estimation*.
