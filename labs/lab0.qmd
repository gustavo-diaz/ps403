...
This lab will help you set up R and RStudio and create a working directory for this class on your computer or the cloud. We will then connect your working directory to GitHub for version control. You may skip the steps you already completed during math camp.

# Set up R and RStudio

We will use R and RStudio to work on lab assignments this quarter. If you have do not have them already, follow the instructions [here](https://rcdsdocs.it.northwestern.edu/tutorials/r/r-installation.html). If you have a Chromebook, check [here](https://levente.littvay.hu/chromebook/).

If your version of R is older than 4.6.x, then you should consider uninstalling it and starting with a fresh install. RStudio will prompt you once a new version is available and give you instructions to update. If you have used RStudio in the past, I strongly recommend you take the time to update R and RStudio now.
...
1. Launch RStudio and open the `.qmd` version of this lab. The default name is \path{lab0.qmd}. It is a good idea to not modify the naming convention for now.

2. To create PDF reports for assignments in this class, you need to have a version of \LaTeX{} installed. \LaTeX{} is a typesetting engine, and we won't use it directly in this class, but R will call it under the hood to generate reports. We will use [TinyTex](https://yihui.org/tinytex/), an R-friendly \LaTeX distribution. Running the code below **in a terminal panel** will install TinyTex. If you do not see a terminal tab, navigate to `Tools > Terminal > New Terminal`.^[**You need to do this once after a fresh installation of R and RStudio.** You may skip this step in Posit Cloud. Check the [tinyTex](https://yihui.org/tinytex/) documentation for more instructions.]

<!-- Center and give a gray background -->
...
*If you are have R and RStudio installed and can successfully render PDFs, you can start in this step.*

Now we will create a working directory that we will use to keep all of the assignments for this course organized:
...
If you already have connected GitHub and RStudio (we completed these steps during math camp), you can skip this section. If you have not done so already, you can follow the instructions [here](https://gustavodiaz.org/NUmathcamp/r/06_github.html).

Just to make sure we have everything in working order, the following code will show whether a GitHub PAT is detected.
...
4. Back to RStudio, locate the *Terminal* tab. Remember, if you do not see it, navigate to `Tools > Terminal > New Terminal`
