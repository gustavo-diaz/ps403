...
AM Chapter 2 introduces many summary features of random variables. We have also discussed how one nice feature of random variables is that they are technically functions but we can treat them as variables. This means the quantities introduced in the chapter also have an analogue that we can compute with actual data. Knowing this will be useful later to visualize the statistical properties of different *estimators*.^[This language is slowly filtering into our vocabulary, we will formalize it later.]
...
- The expected value, variance, and standard deviation of the first variable (feel free to use base R functions for these)
- The expected value, variance, and standard deviation of the second variable
...
- Present a quantities at a meaningful level of precision (rounding if necessary)
