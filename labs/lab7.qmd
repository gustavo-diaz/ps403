...
Finally, we can also conduct randomization inference, and permutation testing in general, with the `mosaic` package. Notice the use of the `shuffle()` function.
...
What if you wanted to compare between groups, instead of each group by the baseline? You can adapt your `lm_robust` code to conduct *linear hypotheses* using `lh_robust`.
...
## Explain how every variable (other than `messages`) in `social` is measured.

## Customize the `conditions` table above to make it more informative.

## Interpret the result of `ate` in terms of the estimate, confidence intervals, and p-value (print a nice table if you can)
...
## Compare all possible pairwise combinations of treatment groups using `lh_robust`. Is there a particular message that is more effective than others?
