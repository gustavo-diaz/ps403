...
::: aside
You can also interpret it as the interval containing all the hypotheses we cannot reject at our chosen $\alpha$. This interpretation sees CIs as **inverted hypothesis tests** (more if we have time)
:::
...
2. **Actual coverage:** _Empirical_ coverage probability
...
::: aside
No good reason for $\alpha = 0.05$ other than path dependency. You may choose to just interpret raw p-values as evidence against the null.
:::
...
::: aside
The $t$ statistic/distribution is analogous to the $z$ statistic/distribution. The first is attributed to William Gosset (writing as Student), the second is traced back to Carl Friedrich Gauss.
:::
...
```{r}
gss %>% t_test(response = vote) %>% select(estimate, statistic, p_value)
```

. . .

```{r}
gss %>% t_test(response = vote, mu = 0.5) %>% select(estimate, statistic, p_value)
```
...
hyps = seq(0, 1, by = 0.001) # 1k hypotheses
```
...
```{r}
gss %>% t_test(response = vote) %>% select(lower_ci, upper_ci)
```
